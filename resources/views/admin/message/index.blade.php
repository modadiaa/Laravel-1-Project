        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    ...
                    <div class="col-sm-6">
                        <form method="GET" action="{{ route('dashboard.messages.index') }}">
                    ...
                    <div class="col-sm-3">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">{{ __('words.messages') }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr id="mess_ids{{ $mess->id }}">
                                <th><input type="checkbox" class="checkbox_ids" name="ids" value="{{ $mess->id }}" id="" /></th>
                                <td>{{ $mess->id }}</td>
                                <td>{{ $mess->name }} </td>
                                <td>{{ $mess->email }} </td>
                                <td>{{ $mess->subject }} </td>
                                ...
                                <td><a href="{{ route('dashboard.messages.destroy', ['id' => $mess->id]) }}"
                                        class="btn btn-block btn-danger btn-sm"
                                        onclick="return confirm('{{ __('words.sure-delete') }}')">{{ __('words.delete') }}</a> </td>
                                <td><a href="{{ route('dashboard.messages.show', ['id' => $mess->id]) }}"
    ...
    <script>
        $(function(e){
          ...
          $('#deleteAllSelectedRecord').click(function(e){
             ...
             $.ajax({
                 url:"{{ route('dashboard.messages.deleteall') }}",
                 type:"DELETE",
                 data:{
                     ids:all_ids,
                     _token:'{{ csrf_token() }}'
