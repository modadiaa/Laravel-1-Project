    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-3">
                        <a href="{{ route('dashboard.news.create') }}" class="btn btn-block bg-gradient-info"
                            style="width: 200px">  {{ __('words.add-news') }}</a>

                    </div>
                    <div class="col-sm-6">
                        <form method="GET" action="{{ route('dashboard.news.index') }}">
                    ...
                    <div class="col-sm-3">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">{{ __('words.news') }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr id="news_ids{{ $news->id }}">
                                <th><input type="checkbox" class="checkbox_ids" name="ids" value="{{ $news->id }}" id="" /></th>
                                <td>{{ $news->id }}</td>
                                <td>{{ $news->title }} </td>
                                <td>{!! $news->description !!} </td>
                                <td>
                                    @if ($news->image)
                                        <img src="{{ Storage::url($news->image) }}" style="height: 40px">
                                    @endif

                                </td>
                                <td><a href="{{ route('dashboard.news.edit', ['id' => $news->id]) }}"
                                        class="btn btn-block btn-info btn-sm">{{ __('words.edit') }}</a> </td>
                                <td><a href="{{ route('dashboard.news.destroy', ['id' => $news->id]) }}"
                                        class="btn btn-block btn-danger btn-sm"
                                        onclick="return confirm('{{ __('words.sure-delete') }}')">{{ __('words.delete') }}</a> </td>
                                <td><a href="{{ route('dashboard.news.show', ['id' => $news->id]) }}"
