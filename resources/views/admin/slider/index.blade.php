    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-3">
                        <a href="{{ route('dashboard.slider.create') }}" class="btn btn-block bg-gradient-info"
                            style="width: 200px">  {{ __('words.add-slider') }}</a>
                    </div>
                    ...
                    <div class="col-sm-3">
                        <ol class="breadcrumb float-sm-right">
                            ...
                            <li class="breadcrumb-item active">{{ __('words.slider') }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
                ...
                <div class="card-body">
                    <table class="table table-bordered">
                        ...
                        <tbody>
                            ...
                            <tr id="slider_ids{{ $slider->id }}">

                                <th><input type="checkbox" class="checkbox_ids" name="ids" value="{{ $slider->id }}" id="" /></th>
                                <td>{{ $slider->id }}</td>
                                <td>{{ $slider->title }} </td>
                                <td>{{ $slider->description }} </td>
                                <td>
                                    @if ($slider->image)
                                        <img src="{{ Storage::url($slider->image) }}" style="height: 40px">
                                    @endif

                                </td>
                                <td>{{ $slider->link }} </td>
                                <td><a href="{{ route('dashboard.slider.edit', ['id' => $slider->id]) }}"
                                        class="btn btn-block btn-info btn-sm">{{ __('words.edit') }}</a> </td>
                                <td><a href="{{ route('dashboard.slider.destroy', ['id' => $slider->id]) }}"
                                        class="btn btn-block btn-danger btn-sm"
                                        onclick="return confirm('{{ __('words.sure-delete') }}')">{{ __('words.delete') }}</a> </td>
                                <td><a href="{{ route('dashboard.slider.show', ['id' => $slider->id]) }}"
    ...
    <script>
        $(function(e){
          ...
          $('#deleteAllSelectedRecord').click(function(e){
             ...
             $.ajax({
                 url:"{{ route('dashboard.slider.deleteall') }}",
                 type:"DELETE",
                 data:{
                     ids:all_ids,
                     _token:'{{ csrf_token() }}'
