...
@section('title', 'Edit Slider :' . $data->title)
    ...
    <div class="content-wrapper">
        ...
        <section class="content-header">
            <div class="container-fluid">
                <div class="row mb-2">
                    <div class="col-sm-6">
                        <h1>{{ __('words.edit-slider') }} : {{ $data->title }}</h1>
                    </div>
                    <div class="col-sm-6">
                        <ol class="breadcrumb float-sm-right">
                            <li class="breadcrumb-item"><a href="">{{ __('words.home') }}</a></li>
                            <li class="breadcrumb-item active"> {{ __('words.edit-slider') }}</li>
                        </ol>
                    </div>
                </div>
            </div><!-- /.container-fluid -->
        ...
        <section class="content">
            ...
            <div class="card card-primary">
                ...
                <!-- /.card-header -->
                <!-- form start -->
                <form role="form" action="{{ route('dashboard.slider.update', ['id' => $data->id]) }}" method="post"
                            ...
                            @foreach (config('app.languages') as $key => $lang)
                                <div class="tab-pane mt-3 fade @if ($loop->index == 0) show active in @endif"
                                    ...
                                    <div class="form-group col-md-12">
                                        ...
                                        <textarea name="{{$key}}[description]" class="form-control" cols="30" rows="10">{{$data->translate($key)->description}}</textarea>
                                    </div>
                                </div>
                            @endforeach
                        </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="form-group mt-3 col-md-12">
                            <label>{{ __('words.link') }} - {{ $lang }}</label>
                            <input type="text" name="link" class="form-control"
                                placeholder="{{ __('words.link') }}" value="{{$data->link}}">
                        </div>
